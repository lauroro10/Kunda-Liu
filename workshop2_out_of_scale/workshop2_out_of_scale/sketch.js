...
// Steering values coming from phone tilt
let steerX = 0;
let steerY = 0;
...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    // Noise field plus tilt values from the phone
  ...
  // Small random pulses keep the audio alive when the system is active
...
function setupSocket() {
  if (typeof io === "undefined") {
    updateStatus("socket.io client missing");
    return;
  }
...
function setupUI() {
  ...
  // Desktop mouse / touch also inject energy for testing
...
async function enableSensors() {
  try {
    ...
    updateStatus("sensor permission denied or unavailable");
    return false;
  }
}
...
function updateStatusText() {
  ...
  const sensorState = millis() - lastSensorAt < 1500 ? "live" : sensorsEnabled ? "waiting" : "off";
  const audioState = audioReady ? "on" : "off";

  statusEl.textContent = `socket: ${sock} • energy: ${energy.toFixed(2)} • audio: ${audioState} • sensor: ${sensorState}`;
}
